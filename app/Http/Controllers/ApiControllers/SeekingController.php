...
class SeekingController extends Controller
{
	...
	public function get_users(request $request)
    {
        ...
        if($preferences->global == false || array_key_exists('filter_enabled', $format_filter)){
          $peoples->wherehas('Location', function ($query) use($user) {
            $query->where('zipcode',$user->zipcode);
          });
        }
      ...
      if(!empty($format_filter["visibility"]) && $format_filter["visibility"][0] == 'last24hours')  
        ...
        if(!empty($format_filter["visibility"]) && $format_filter["visibility"][0] == 'have_photos')  
          ...
          if(!empty($format_filter["spiritualValue"]))  
          {
            $peoples->WhereIn("spiritualValue",$format_filter["spiritualValue"]);
          }
          ...
          if($preferences->global == false || array_key_exists('filter_enabled', $format_filter)){
             $peoples->having("distance","<=", $format_filter["miles"][0]);
          }
          $peoples->orderby("spotlight_status","desc")
	...
	//saad code
	public function youaretheretype(request $request)
    {
      $user = Auth::user(); 
      $Location = $user->Location()->first();
      $preferences = $user->preferences()->first();
