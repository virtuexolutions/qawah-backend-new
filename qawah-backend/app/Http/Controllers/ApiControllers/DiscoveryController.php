...
use App\User;
use App\User_Activate_Spotlight;
use App\UserAction;
...
class DiscoveryController extends Controller
{
    ...
    public function get_peoples(request $request)
    {
      ...
      // ->where('mobile_verified', '=', true) //saad comment
      // ->where('email_verified', '=', true) //saad comment
    ...
    public function liked(request $request)
    {
      ...
      if(empty($r2)){
        ...
        $response["only"] = $user->useractions_by_auth_user()->create(
          [
       ...
       $response["notification_1"] = $this->send_notifications(array("title"=> $match_user->profileName." Kan by you" , "body" =>  $match_user->profileName." Kan by you" ));
       $response["notification_2"] =  $this->send_target_notifications($match_user->id,array("title"=> $user->profileName." Kan You " , "body" =>  $user->profileName." Kan You "));
    ...
    public function rewind(request $request)
    {
         ...
         $reponse["target_user"] = $user->useractions()->where("user_id",$request->targetsUid)->delete();
         $myuid = $user->uid;
